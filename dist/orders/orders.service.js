...
let OrdersService = class OrdersService {
    ...
    getOrders({ first, page, customer_id, tracking_number, shop_id, }) {
        var _a;
    ...
    getOrder({ id, tracking_number }) {
        let parentOrder = undefined;
        if (id) {
            parentOrder = this.orders.find((p) => p.id === Number(id));
        }
        else {
            parentOrder = this.orders.find((p) => p.tracking_number === tracking_number);
        }
        if (!parentOrder) {
            return this.orders[0];
        }
    ...
    getOrderStatuses({ first, page, text, orderBy, }) {
        const startIndex = (page - 1) * first;
        const endIndex = page * first;
